function toDirectMediaUrl(url) {
  ...
  const driveId = normalizedUrl.match(/drive\.google\.com\/file\/d\/([^/?]+)/)?.[1] || normalizedUrl.match(/drive\.google\.com\/thumbnail\?id=([^&]+)/)?.[1] || normalizedUrl.match(/lh3\.googleusercontent\.com\/d\/([^=/?]+)/)?.[1] || (isGoogleDrive ? normalizedUrl.match(/[?&]id=([^&]+)/)?.[1] : void 0);
...
export {
  applyMediaFallback as a,
  getMediaUrlCandidates as g
};
