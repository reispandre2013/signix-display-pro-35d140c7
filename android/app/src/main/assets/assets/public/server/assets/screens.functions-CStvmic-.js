...
const claimPairingCode = createServerFn({
  ...
  .handler(claimPairingCode_createServerFn_handler, async ({ data }) => {
    ...
    if (!pairing)
      throw new Error("Código não encontrado. Verifique se a TV ainda exibe este código.");
...
const checkPairingStatus = createServerFn({
  ...
  .handler(checkPairingStatus_createServerFn_handler, async ({ data }) => {
    try {
