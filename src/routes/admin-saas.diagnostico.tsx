...
import { CheckCircle2, XCircle, AlertTriangle, ShieldCheck, Loader2, RefreshCw, Sparkles } from "lucide-react";
          ...
          <div className="flex items-start gap-2 text-destructive">
            ...
            <div>
              ...
              <p className="text-xs mt-2 text-muted-foreground">
                Verifique se <code>SERVICE_ROLE_KEY</code> está configurada nas variáveis do servidor.
              </p>
            </div>
          </div>
        </Panel>
        ...
        <>
          <Panel title="Sessão atual">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Sem este papel as páginas <code>/admin-saas</code> exibem estados vazios porque as
                      RLS bloqueiam SELECTs. Clique abaixo para se promover usando a service role no servidor.
                    </p>
                    <button
            ...
            {data.isSuperAdmin && (
              <div className="mt-4 rounded-lg border border-success/30 bg-success/5 p-3 flex items-center gap-2">
                ...
                <span className="text-sm">Tudo certo — seu usuário tem acesso total ao painel SaaS.</span>
              </div>
            )}
          </Panel>
          ...
          <Panel title="Saúde das tabelas SaaS">
            <p className="text-xs text-muted-foreground mb-3">
              Contagem real (via service role, sem RLS). Se algum valor for <code>0</code>,
              as páginas correspondentes vão mostrar estados vazios — popule a base ou rode as seeds.
            </p>
              ...
              {Object.entries(data.samples).map(([table, rows]) => (
                <details key={table} className="rounded-md border border-border bg-surface/30">
                  <summary className="cursor-pointer px-3 py-2 text-xs font-mono flex items-center gap-2">
                    ...
                    {table} <span className="text-muted-foreground">({(rows as unknown[]).length})</span>
                  </summary>
