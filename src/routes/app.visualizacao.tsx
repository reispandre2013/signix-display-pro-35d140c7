  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard label="Online" value={online} icon={Wifi} tone="success" hint={screens.length ? `${Math.round((online / screens.length) * 100)}%` : "—"} />
        <KpiCard label="Offline" value={offline} icon={WifiOff} tone="destructive" />
        <KpiCard label="Campanhas em execução" value={activeCampaigns.length} icon={Megaphone} tone="info" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Panel
          title="Campanhas em execução"
          actions={<Link to="/app/campanhas" className="text-[11px] text-primary hover:underline inline-flex items-center gap-1">Ver detalhes <ArrowRight className="h-3 w-3" /></Link>}
        >
          {activeCampaigns.length === 0 ? (
            <p className="text-sm text-muted-foreground py-4 text-center">Nenhuma campanha em execução.</p>
          ) : (
        ...
        >
          {screens.length === 0 ? (
            <p className="text-sm text-muted-foreground py-4 text-center">Nenhuma tela cadastrada.</p>
          ) : (
            <ul className="divide-y divide-border -my-3">
              {screens.slice(0, 6).map((s) => (
                <li key={s.id} className="flex items-center gap-3 py-3">
                  <div className={`h-9 w-9 rounded-lg grid place-items-center ${s.is_online ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>
      ...
      <Panel title="Relatórios disponíveis" description="Acesse os relatórios consolidados.">
        <Link to="/app/relatorios" className="inline-flex items-center gap-2 rounded-md border border-border px-3 py-2 text-sm hover:bg-surface">
