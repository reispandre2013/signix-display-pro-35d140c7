        ...
        <div className="text-center max-w-2xl mx-auto">
          ...
          <div className="mt-6 inline-flex items-center gap-1 rounded-full border border-border bg-card p-1">
            <button
              ...
              className={cn(
                "px-4 py-1.5 text-xs font-semibold rounded-full transition-smooth",
        ...
        {isError && (
          <p className="mt-8 text-center text-sm text-destructive" role="alert">
            {error instanceof Error
              ? error.message
              : "Não foi possível carregar os planos. Verifique a ligação e as migrations SaaS."}
          </p>
        )}
          ...
          <div className="mt-12 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            {list.length === 0 && !isError ? (
              <p className="col-span-full text-center text-sm text-muted-foreground py-8">
                Nenhum plano publicado. Configure planos ativos no Supabase (tabela{" "}
                <code className="text-xs">public.plans</code>).
              </p>
            ) : null}
            {list.map((p) => {
              const price =
                cycle === "monthly" ? p.price_monthly_cents : Math.round(p.price_yearly_cents / 12);
              return (
                <div
                  key={p.id}
                  className={cn(
                  ...
                  <div className="mt-5">
                    ...
                    {cycle === "yearly" && (
                      <p className="text-[11px] text-success font-semibold mt-1">
                        Cobrado {formatPrice(p.price_yearly_cents)}/ano
                      </p>
                    )}
                  </div>
        ...
        <section className="mt-16 max-w-3xl mx-auto">
          ...
          <div className="mt-6 space-y-3">
            {[
              ...
              <details
                key={f.q}
                className="rounded-lg border border-border bg-card px-4 py-3 group"
              >
