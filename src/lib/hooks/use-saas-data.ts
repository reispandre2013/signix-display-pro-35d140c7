...
export function useOrgBillingContext(): {
  ...
  const q = useQuery({
    ...
    queryFn: async (): Promise<OrgBillingBundle> => {
      ...
      const subscription = subRow ? mapSubscriptionRow(subRow, planData ?? null) : null;
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<SaasClient[]> => {
      ...
      return orgs.map((org) => {
        ...
        const planEmbed = latest?.plans as
          | { name?: string; max_screens?: number }
          | { name?: string; max_screens?: number }[]
          | null;
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<PaymentRow[]> => {
      const { data, error } = await supabase
        .from("payments")
        .select(
          "id, amount_cents, status, created_at, method, organization_id, organizations(name)",
        )
        .order("created_at", { ascending: false })
        .limit(limit);
      if (error) {
      ...
      return (data ?? []).map((r) => {
        const o = (r as { organizations?: { name: string } | { name: string }[] | null })
          .organizations;
        ...
        return {
          ...
          status:
            String((r as { status: string }).status) === "failed" ||
            String((r as { status: string }).status) === "pending"
              ? "pending"
              : "paid",
...
export function useSaasMetrics() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<SaasMetrics> => {
      ...
      const overdue = (inv.data ?? []).filter(
        (i) => (i as { status: string }).status === "overdue",
      ).length;
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AuditListItem[]> => {
      ...
      const profIds = [
        ...new Set(
          (data ?? [])
            .map((r) => (r as { actor_profile_id: string | null }).actor_profile_id)
            .filter(Boolean),
        ),
      ] as string[];
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<LicenseTableRow[]> => {
      const { data, error } = await supabase
        .from("licenses")
        .select(
          "id, license_key, key, status, max_screens, valid_from, valid_until, plans(name), organizations(name)",
        )
        .order("valid_from", { ascending: false });
      if (error) {
      ...
      return (data ?? []).map((row) => {
        ...
        return {
          ...
          key: String(
            (row as { license_key?: string; key?: string }).license_key ??
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<SubscriptionsTableRow[]> => {
      ...
      const { data: payAll } = await supabase
        ...
        .order("created_at", { ascending: false });
      const orgIds = [
        ...new Set((data ?? []).map((r) => (r as { organization_id: string }).organization_id)),
      ];
      const lastPaid = new Map<string, string | null>();
      for (const oid of orgIds) {
        const t = (payAll ?? []).filter(
          (p) =>
            p.organization_id === oid &&
            (p as { paid_at: string | null }).paid_at &&
            String((p as { status: string }).status) !== "failed",
        ) as { paid_at: string; created_at: string }[];
        t.sort((a, b) => new Date(b.paid_at).getTime() - new Date(a.paid_at).getTime());
      ...
      return (data ?? []).map((row) => {
        ...
        return {
          ...
          status: String((row as { status: string }).status),
          billing_cycle:
            (row as { billing_cycle: string }).billing_cycle === "yearly" ? "Anual" : "Mensal",
          amount_cents: Number((row as { amount_cents: number }).amount_cents),
          current_period_start:
            (row as { current_period_start: string | null }).current_period_start ??
